...
The JMS configuration for Merative Social Program Management (SPM) specifies a hierarchy of queues, from the normal queue, to an error queue, and finally to the dead message queue.
...
For instance, Cúram Express Rules processing will transfer messages routed to the JMS error queue to batch as discussed in [Dependency Manager deferred processing](https://www.ibm.com/docs/en/spm/8.0.2?topic=manager-dependency-deferred-processing).
