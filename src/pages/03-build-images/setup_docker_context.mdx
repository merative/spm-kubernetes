...
* The `release.zip` and `StaticContent.zip` files that you built in [Building the SPM application](/02-deploy-spm/spm-build).

## Building IBM® Social Program Management (SPM) resources

Some of the Docker images that you build rely on the `release.zip` and `StaticContent.zip` files that you built in [Building the SPM application](/02-deploy-spm/spm-build).
...
All configuration of WebSphere Liberty will be handled by the Helm Charts. See the [Preparing Helm Charts](/04-deployment/hc_preparation) topic for more information.
...
**Note:** As of Release 20.7.0, the IBM MQ Resource Adapter version used to verify this runbook is 9.1.5.0.
...
1. Download the IBM MQ resource adapter from Fix Central.
    * Click this link: [IBM MQ Resource Adapter](https://ibm.biz/mq91cdRA)
    * Find the resource adapter for your version of IBM MQ in the displayed list of available fixes. For example:

      ```
      release level: 9.1.5.0-IBM-MQ-Java-InstallRA
