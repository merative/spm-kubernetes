...
Note the use of `$ANT_HOME` in the `docker run` command above. The value must map to a working copy of Ant on your workstation, which can be extracted from `$SPM_HOME/dockerfiles/Liberty/content/apache-ant-1.10.6-bin.zip`:
