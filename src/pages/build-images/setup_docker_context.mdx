...
* The `release.zip` and `StaticContent.zip` files that you built in [Building the SPM application](/deploy-spm/spm-build).

## Building IBM® Social Program Management (SPM) resources

Some of the Docker images that you build rely on the `release.zip` and `StaticContent.zip` files that you built in [Building the SPM application](/deploy-spm/spm-build).
