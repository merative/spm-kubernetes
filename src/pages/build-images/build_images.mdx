...
If you do not have an active RedHat subscription, or prefer to build these images from source, see [building Base images](../base_images).
...
In the following commands, `$PROJECT` refers to the name of the project in use:
...
```shell
curl -k -ukubeadmin:$(oc whoami -t) https://default-route-openshift-image-registry.apps-crc.testing/v2/_catalog
```
