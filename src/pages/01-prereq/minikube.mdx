...
Minikube runs a single-node Kubernetes cluster inside a virtual ma
chine (VM) on your laptop, for users who want to try out Kubernetes or develop with it.
...
**Note:** Minikube version used to verify this runbook: 1.7.3
...
```shell
minikube start --vm-driver=virtualbox --cpus 4 --memory 8G --insecure-registry "192.168.0.0/16" --disk-size='30G' --kubernetes-version v1.16.7
```
...
To use Helm commands on Minikube, initialize Tiller, which is the Helm server pod within Kubernetes, by running the following commands:
...
Both provide _translation_ of domain names to a LAN address, e.g. a domain of `hello-world.192.168.99.100.nip.io` will behave as if you have entered `192.168.99.100` (replace the IP with your own Minikube IP!) in the address bar.
``
