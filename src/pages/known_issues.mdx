...
(see the SPM [Kubernetes architecture](https://www.ibm.com/docs/en/spm/8.0.0?topic=kubernetes-architecture) documentation for more information).
...
For SPM product classes that have this requirement the Liberty shared folder is used.
These product classes are the EJB local interfaces as described in the SPM [Transaction isolation]( https://www.ibm.com/docs/en/spm/8.0.0?topic=architecture-transaction-isolation) documentation.

In relation to the Liberty classloader for SPM, SPM specifies the `parentFirst` classloader policy. This was done to increase cohesion and lower coupling between the client and server.
