...
docker build \
     --tag minikube.local:5000/xmlserver:latest \
...
docker build \
     --tag minikube.local:5000/batch:latest \
...
docker build \
     --tag minikube.local:5000/curam:latest \
     --file EAR.Dockerfile \
...
```shell
docker build \
     --tag minikube.local:5000/citizenportal:latest \
     --file EAR.Dockerfile \
     --build-arg MQ_RA_LICENSE=--acceptLicense \
...
docker build \
     --tag minikube.local:5000/mqserver:latest \
     --file Dockerfile .
```
...
docker build \
     --tag minikube.local:5000/ihs:latest \
...
cd $SPM_HOME/dockerfiles/HTTPServer/
docker build \
     --tag minikube.local:5000/ce-app:latest \
     --file CE.Dockerfile .
```
...
```shell
docker push minikube.local:5000/citizenportal:latest
docker push minikube.local:5000/ce-app:latest
```
