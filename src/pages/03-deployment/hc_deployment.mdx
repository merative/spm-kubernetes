...
However, as described in [Setting up the Docker® context](../02-build-images/setup_docker_context), you can override the configuration value by running the following shell script:
...
Here are two sample override files for deploying to Minikube and to IBM® Kubernetes Service (IKS).

<InlineNotification>

While the Minikube configuration is sufficiently generic to be used almost as-is (given all images were built and pushed with the names outlined in the [Building Docker images](../02-build-images/build_images) section):
...
The application can also be accessed by using a service port directly. To do so, identify the name of the service, to which you want to connect. Run the following command:
...
This command provides an IP address and a port to connect to.  To use the provided URL, replace  `http` with `https`. Point your browser to that URL to open the SPM login page.

When running `minikube service spm-apps-curam --url` on Red Hat, you might encounter an error
