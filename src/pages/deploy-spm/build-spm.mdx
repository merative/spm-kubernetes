...
**Note:** It is not required to run he `precompile` target on the `CuramServerCode.ear`, `CuramWebServices.ear` or `Rest.ear`.
...
On OSX and Red Hat a zip file will end with .zip and look like a white file.

On Windows a zip file is end with .zip and look like a folder.
