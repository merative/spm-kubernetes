...
| `global.apps.common.persistence.storageAccessModes` | Modes of the storage access (commonly `ReadWriteMany` or `ReadWriteOnce`) | `''` |
| `global.apps.common.persistence.persistentVolumeReclaimPolicy` | `Retain` or `Delete` the container after the persistence volume clain is deleted  | `''` |
| `global.apps.common.persistence.persistentVolumeCsiDriver` | CSI Driver to use for the persistent Volume | `''` |
| `global.apps.common.persistence.persistentVolumeContainer` | Name of the container to use | `''` |
| `global.apps.common.persistence.persistentVolumeHandle` | Unique handle ID across every identical storage blob container in the cluster | `''` |
...
[bulk reassessment of all open integrated cases](https://www.ibm.com/docs/en/spm/8.0.2?topic=reform-running-bulk-reassessment-all-open-integrated-cases)
...
Merative SPM supports MQ on a VM for both Openshift and Kubernetes Service.
...
| ![SPM 8.0.1.0](https://img.shields.io/badge/-SPM_8.0.1.0-green) `xmlserver.metrics.enabled`| Enable sending XML server statistics (`stats/ThreadPoolWorker-*` file contents) to Prometheus | `false` |
| ![SPM 8.0.1.0](https://img.shields.io/badge/-SPM_8.0.1.0-green) `xmlserver.metrics.port`| The port to monitor the XML server statistics | `8080` |
| ![SPM 8.0.1.0](https://img.shields.io/badge/-SPM_8.0.1.0-green) `xmlserver.startOptions` | XML server start option; specifically for specifying `-forcestatswrite`, described in [Statistics](https://www.ibm.com/docs/en/spm/8.0.2?topic=server-statistics) | `''` |
...
| ![SPM 8.1.0.0](https://img.shields.io/badge/-SPM_8.1.0.0-green) `xmlserver.readinessProbe.enabled` | Enable XML server readinessProbe described in [XML server health checks](/monitoring/xmlserver-monitoring#xml-server-health-checks) | `false` |
| ![SPM 8.1.0.0](https://img.shields.io/badge/-SPM_8.1.0.0-green) `xmlserver.readinessProbe.initialDelaySeconds` | Configure the number of seconds after container startup to initiate the probe | `15` |
| ![SPM 8.1.0.0](https://img.shields.io/badge/-SPM_8.1.0.0-green) `xmlserver.readinessProbe.periodSeconds` | Configure the number of seconds between probe invocations | `60` |
| ![SPM 8.1.0.0](https://img.shields.io/badge/-SPM_8.1.0.0-green) `xmlserver.livenessProbe.enabled` | Enable XML server livenessProbe described in [XML server health checks](/monitoring/xmlserver-monitoring#xml-server-health-checks) | `false` |
| ![SPM 8.1.0.0](https://img.shields.io/badge/-SPM_8.1.0.0-green) `xmlserver.readinessProbe.initialDelaySeconds` | Configure the number of seconds after container startup to initiate the probe | `30` |
| ![SPM 8.1.0.0](https://img.shields.io/badge/-SPM_8.1.0.0-green) `xmlserver.readinessProbe.periodSeconds` | Configure the number of seconds between probe invocations | `30` |
| ![SPM 8.1.0.0](https://img.shields.io/badge/-SPM_8.1.0.0-green) `xmlserver.readinessProbe.timeoutSeconds` | Configure the number of seconds before the probe times out | `10` |
...
| `uawebapp.ingressPath` | Static content Ingress/Route path without a trailing forward slash (must match the [PUBLIC_URL](https://www.ibm.com/docs/en/spm/8.0.2?topic=server-building-universal-access-responsive-web-application-deployment) variable when the application was built) | `/universal` |
