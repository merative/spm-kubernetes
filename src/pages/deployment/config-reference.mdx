...
This page contains all available configuration options built into the reference Helm charts for SPM.
...
The tables below list the configurable parameters of the `spm` chart and their default values.

* [Global properties](#global-properties)
* [Liberty runtime](#liberty-runtime)
* [Batch jobs](#batch-jobs)
* [IBM MQ](#ibm-mq)
* [Static Content Server](#static-content-server)
* [XML Server](#xml-server)
* [Universal Access (React App)](#universal-access-(react-app)-optional) - optional
...
In the properties below, `<applicationID>` is a placeholder for the name of SPM applications in lower case (e.g. `curam`, `citizenportal`, `rest`, etc).
...
| ![SPM version](https://img.shields.io/badge/SPM-7.0.10.0-blue) `global.openldap.enabled` | Deploy OpenLDAP alongside SPM as the identity provider | `false` |
...
In the properties below, `<progID>` is a placeholder for the name of the batch program - this must be a valid YAML term, but does not need to match the Batch process class.
...
| `batch.programs.<progID>.javaOptions` | Java options to be injected via `ANT_OPTS` | `-Xms1g -Xmx1g` |
...
The above example adds the [bulk reassessment of all open integrated cases](https://www.ibm.com/support/knowledgecenter/SS8S5A_7.0.10/com.ibm.curam.content.doc/HCRDeveloper/t_HCR_Batch_Runningbulkreassessmentallopenintegratedcases.html)
...
| Parameter | Description | Default |
| --------- | ----------- | ------- |
| ![Container only](https://img.shields.io/badge/-OpenShift_only-red) `global.mq.version` | Version of [IBM MQ](https://hub.docker.com/r/ibmcom/mq) to deploy | `9.1.5.0-r2` |
| ![VM based only](https://img.shields.io/badge/-IKS_only-blue) `global.mq.useConnectionNameList` | Indicator if IBM MQ is running in HA mode | `false` |
| ![Container only](https://img.shields.io/badge/-OpenShift_only-red) `global.mq.queueManager.name` | Name of the queue manager to be created in the MQ container | `QM1` |
...
| ![Container only](https://img.shields.io/badge/-OpenShift_only-red) `global.mq.queueManager.secret.adminPasswordKey` | The key in the Kubernetes Secret containing the password for the MQ admin user | `adminPasswordKey` |
...
| ![Container only](https://img.shields.io/badge/-OpenShift_only-red) `global.mq.multiInstance.cephEnabled` | Deploy MQ as a StatefulSet using Ceph storage backend (`cephEnabled` and `nfsEnabled` are mutually exclusive) | `false` |
| ![Container only](https://img.shields.io/badge/-OpenShift_only-red) `global.mq.multiInstance.storageClassName` | Storage class used for dynamic provisioning on the Ceph backend | `''` |
| ![Container only](https://img.shields.io/badge/-OpenShift_only-red) `global.mq.multiInstance.nfsEnabled` | Deploy MQ as a StatefulSet using NFS storage backend (`cephEnabled` and `nfsEnabled` are mutually exclusive) | `false` |
| ![Container only](https://img.shields.io/badge/-OpenShift_only-red) `global.mq.multiInstance.nfsIP` | IP address or hostname of the NFS server | `''` |
| ![Container only](https://img.shields.io/badge/-OpenShift_only-red) `global.mq.multiInstance.nfsFolder` | Path on the NFS server, where MQ data should be stored | `''` |
| ![Container only](https://img.shields.io/badge/-OpenShift_only-red) `global.mq.security.context.fsGroup` | Primary user group for any filesystem mounts, if required by the storage provisioner | `''` |
| ![Container only](https://img.shields.io/badge/-OpenShift_only-red) `global.mq.security.context.supplementalGroups` | Supplemental user groups for any filesystem mounts, if required by the storage provisioner | `[]` |

### Static Content Server

| Parameter | Description | Default |
| --------- | ----------- | ------- |
| `web.ingressPath` | Static content Ingress/Route path without a trailing forward slash (must match [what had been defined](/deploy-spm/spm-build#set-up-static-content) when the application was built) | `/CuramStatic` |
...
| `uawebapp.ingressPath` | Static content Ingress/Route path without a trailing forward slash (must match the [PUBLIC_URL](https://www.ibm.com/support/knowledgecenter/SS8S5A_7.0.10/com.ibm.curam.universalaccess.doc/CitizenEngagement/t_CEINST_WebApp_depl.html) variable when the application was built) | `/universal` |
