...
The above command keeps all the values used with the original `helm install` due to the `--reuse-values` flag, and only changes the `images.tag` value, so all new images are deployed with the `vNext` tag.
