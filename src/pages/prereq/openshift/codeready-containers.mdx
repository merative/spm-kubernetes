...
In order to push docker images into the crc internal registry the default route needs to be enabled.

Enable the Enable the Image Registry default route:
...
The default route signs the certificate by the OpenShift internal CA, which Docker does not trust by default.
...
oc extract secret/router-ca -n openshift-ingress-operator --keys=tls.crt
